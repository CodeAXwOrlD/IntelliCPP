...
try {
  ...
  if (nativePath) {
    ...
    console.log('[API Handler] Native backend module loaded successfully from', nativePath);
  } else {
    console.warn('[API Handler] Native backend module not found, using mock data');
  }
} catch (e) {
  console.error('[API Handler] Failed to load native module:', e.message);
}

// Mock C++ keywords and STL functions (fallback)
const cppKeywords = [
    ...
    if (endpoint === 'getSuggestions') {
      ...
      if (suggestionEngine) {
        try {
          ...
          console.error('[API Handler] Error getting suggestions from backend:', err.message);
      ...
      if (suggestionEngine) {
        try {
          suggestionEngine.updateSymbols(code);
          const symbolCount = suggestionEngine.getSymbolCount();
          const includedLibraries = suggestionEngine.getIncludedLibraries();
          const symbolTable = suggestionEngine.getSymbolTable();
          ...
          return res.status(200).json({
            ...
            wordCount: code.trim() ? code.trim().split(/\s+/).length : 0
          });
        } catch (err) {
          console.error('[API Handler] Error getting stats from backend:', err.message);
          // Fallback to mock data
        }
      }
      ...
      if (suggestionEngine) {
        try {
          ...
          console.error('[API Handler] Error running code in backend:', err.message);

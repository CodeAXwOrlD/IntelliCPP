...
void Tokenizer::initializeKeywords() {
    keywords = {
        ...
        {"bool", true}, {"void", true}, {"long", true}, {"short", true},
        {"unsigned", true}, {"signed", true}, {"const", true},
        {"volatile", true}, {"static", true}, {"extern", true},
        {"auto", true}, {"register", true}, {"class", true},
        {"struct", true}, {"union", true}, {"enum", true},
        {"typedef", true}, {"using", true}, {"namespace", true},
        {"template", true}, {"typename", true}, {"virtual", true},
        {"public", true}, {"private", true}, {"protected", true},
        {"friend", true}, {"new", true}, {"delete", true},
        {"vector", true}, {"map", true}, {"set", true}, {"string", true},
        {"iostream", true}, {"algorithm", true}
    };
}
...
std::vector<Token> Tokenizer::tokenize(const std::string& code) {
    ...
    while (pos < code.length()) {
        ...
        if (isIdentifierStart(code[pos])) {
            ...
            Token::Type type = isKeyword(word) ? Token::Type::KEYWORD : Token::Type::IDENTIFIER;
            ...
            tokens.push_back({Token::Type::LITERAL, code.substr(start, pos - start), static_cast<int>(start)});
            ...
            tokens.push_back({Token::Type::OPERATOR, std::string(1, code[pos]), static_cast<int>(pos)});
            pos++;
        } else {
            tokens.push_back({Token::Type::PUNCTUATION, std::string(1, code[pos]), static_cast<int>(pos)});
            pos++;
        }
    }
...
std::string Tokenizer::extractTypeFromContext(const std::string& code, int cursorPosition) {
